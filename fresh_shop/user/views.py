...
def user_info(request):
    if request.method == 'GET':
        activate = 'info'
        history = request.session.get('goods_cookies','')
        # history = request.session.get('history')
        # 从本地cookie读取商品的浏览记录
        goods_list = []
        if history != '':
            goods_id_list = history.split(';')
            for goods_id in goods_id_list:
                if goods_id:
                    good = Goods.objects.filter(pk=int(goods_id)).first()
                    goods_list.append(good)
        # 获取地址和联系电话
        user_id = request.session.get('user_id')
        order_user = OrderInfo.objects.filter(user_id=user_id).first()
        return render(request, 'user_center_info.html',
                      {'activate': activate, 'goods_list': goods_list, 'order_user': order_user})
